...
void PathConverter::configure(nav2::LifecycleNode::SharedPtr node)
{
  ...
  density_ = static_cast<float>(nav2::declare_or_get_parameter<double>(node, "path_density", 0.05));
  smoothing_radius_ = static_cast<float>(
    nav2::declare_or_get_parameter<double>(node, "smoothing_radius", 1.0));
  smooth_corners_ = nav2::declare_or_get_parameter<bool>(node, "smooth_corners", false);
