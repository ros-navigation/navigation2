  ...
  const std::string & name)
{
  ...
  double checking_rate = nav2::declare_or_get_parameter(node, getName() + ".rate", 1.0);
  checking_duration_ = rclcpp::Duration::from_seconds(1.0 / checking_rate);

  reroute_on_collision_ = nav2::declare_or_get_parameter(
  ...
  check_resolution_ = static_cast<unsigned int>(
    nav2::declare_or_get_parameter(node, getName() + ".check_resolution", 1));

  max_collision_dist_ = static_cast<float>(
    nav2::declare_or_get_parameter(node, getName() + ".max_collision_dist", 5.0));
