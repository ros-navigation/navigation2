...
{
public:
  explicit BtServiceNode(const std::string & service_name)
  : BT::CoroActionNode(service_name), service_name_(service_name)
  {
  }

  BtServiceNode(const std::string & service_name, const BT::NodeParameters & params)
  : BT::CoroActionNode(service_name, params), service_name_(service_name)
  {
  }
  ...
  void onInit() final
  {
    ...
    // Now that we have the ROS node to use, create the service client for this BT action
    service_client_ = node_->create_client<ServiceT>(service_name_);

    // Make sure the server is actually there before continuing
    RCLCPP_INFO(node_->get_logger(), "Waiting for \"%s\" service", service_name_.c_str());
    service_client_->wait_for_service();

    // Give the derive class a chance to do any initialization
    on_init();
    RCLCPP_INFO(node_->get_logger(), "\"%s\" BtServiceNode initialized", service_name_.c_str());
  }
