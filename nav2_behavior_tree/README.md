...
| SpeedController | Decorator | A node that controls the tick rate for its child based on the current speed. This decorator offers the most flexibility as the user can set the minimum/maximum tick rate which is adjusted according to the current speed. |
