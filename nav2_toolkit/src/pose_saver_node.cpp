...
namespace nav2_toolkit
{
        ...
        was_pose_pub_up_last_check_(false)
  {
    ...
    this->declare_parameter("pose_file_path", std::string(std::getenv("HOME")) + "/last_known_pose.yaml");
  ...
  void PoseSaverNode::post_init_setup()
  {
    ...
    try {
      ...
      set_pose_client_ = std::make_shared<
        nav2_util::ServiceClient<nav2_msgs::srv::SetInitialPose>
      >("set_initial_pose", node_shared);
  ...
  geometry_msgs::msg::PoseWithCovarianceStamped PoseSaverNode::read_pose_from_file(const std::string &path)
  {
  ...
  bool PoseSaverNode::restore_pose_from_file_and_publish()
  {
    ...
    try
    {
      // RCLCPP_INFO(this->get_logger(), "Sending initial pose via nav2_util::ServiceClient...");
      auto response = set_pose_client_->invoke(request, 5s); 
      // RCLCPP_INFO(this->get_logger(), "Pose successfully acknowledged by /set_initial_pose.");
      pose_restored_ = true;
      return true;
    }
    catch (const std::exception &e)
