 ...
 */
class WavefrontMapAccessor : public nav2_costmap_2d::Costmap2D {
  public:
    WavefrontMapAccessor(MapGrid* map, double outer_radius)
      : nav2_costmap_2d::Costmap2D(map->size_x_, map->size_y_, 1, 0, 0),
      ...
      for(unsigned int x = 0; x < size_x_; x++){
        for (unsigned int y = 0; y < size_y_; y++){
          ...
          if(map_->operator ()(x, y).target_dist == 1) {
            costmap_[ind] = nav2_costmap_2d::LETHAL_OBSTACLE;
          } else {
            costmap_[ind] = 0;
          }
        }
