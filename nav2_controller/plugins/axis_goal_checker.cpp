...
: along_path_tolerance_(0.25), cross_track_tolerance_(0.25),
  radial_goal_tolerance_(0.25), path_length_tolerance_(1.0),
  ...
  const std::shared_ptr<nav2_costmap_2d::Costmap2DROS>/*costmap_ros*/)
{
  ...
  radial_goal_tolerance_ = node->declare_or_get_parameter(
    plugin_name + ".radial_goal_tolerance",
  ...
  const nav_msgs::msg::Path & transformed_global_plan)
{
  ...
  if (!cached_end_of_path_yaw_.has_value()) {
    ...
    return distance_to_goal < radial_goal_tolerance_;
  }
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  for (const auto & parameter : parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      ...
      } else if (name == plugin_name_ + ".radial_goal_tolerance") {
        radial_goal_tolerance_ = parameter.as_double();
