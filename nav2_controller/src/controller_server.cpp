    ...
    , default_types_{"dwb_core::DWBLocalPlanner"}
{
    ...
    costmap_ros_ = std::make_shared<nav2_costmap_2d::Costmap2DROS>(
        "local_costmap", std::string{get_namespace()}, "local_costmap", get_parameter("use_sim_time").as_bool());
...
nav2_util::CallbackReturn ControllerServer::on_configure(const rclcpp_lifecycle::State& /*state*/)
{
    ...
    if (goal_checker_ids_ == default_goal_checker_ids_)
    {
        ...
        {
            nav2_util::declare_parameter_if_not_declared(node, default_goal_checker_ids_[i] + ".plugin",
    ...
    if (controller_ids_ == default_ids_)
    {
        ...
        {
            nav2_util::declare_parameter_if_not_declared(node, default_ids_[i] + ".plugin",
    ...
    for (size_t i = 0; i != goal_checker_ids_.size(); i++)
    {
        try
        {
            ...
            RCLCPP_INFO(get_logger(), "Created goal checker : %s of type %s", goal_checker_ids_[i].c_str(),
...
nav2_util::CallbackReturn ControllerServer::on_activate(const rclcpp_lifecycle::State& /*state*/)
{
    ...
    dyn_params_handler_ =
        node->add_on_set_parameters_callback(std::bind(&ControllerServer::dynamicParametersCallback, this, _1));
...
void ControllerServer::computeControl()
{
    ...
    try
    {
        ...
        RCLCPP_WARN(get_logger(), "Current controller %s", c_name.c_str());
        if (findControllerId(c_name, current_controller))
        {
            RCLCPP_WARN(get_logger(), "Updating controller: %s", current_controller.c_str());
        ...
        RCLCPP_WARN(get_logger(), "Current controller %s", gc_name.c_str());
        std::string current_goal_checker;
        if (findGoalCheckerId(gc_name, current_goal_checker))
        {
            RCLCPP_WARN(get_logger(), "Updating goal checker: %s", current_goal_checker.c_str());
...
void ControllerServer::updateGlobalPath()
{
    if (action_server_->is_preempt_requested())
    {
        ...
        {
            RCLCPP_INFO(get_logger(), "Terminating action, invalid goal checker %s requested.",
...
bool ControllerServer::isGoalReached()
{
    ...
    nav_2d_utils::transformPose(costmap_ros_->getTfBuffer(), costmap_ros_->getGlobalFrameID(), end_pose_,
