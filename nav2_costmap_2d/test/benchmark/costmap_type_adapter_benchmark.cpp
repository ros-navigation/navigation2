...
// Benchmark: Before (standard msg + convert_to_custom) vs After (TypeAdapter)
...
// Runs with and without intra-process communication (IPC).
  ...
  const std::vector<double> & samples_us)
{
  ...
  std::printf(
    "  %-48s  mean=%7.1f us  min=%7.1f us  max=%7.1f us\n",
    label.c_str(), mean, mn, mx);
}

// ---------------------------------------------------------------------------
// After: TypeAdapter subscription
// With IPC: zero-copy, no conversion at all
// Without IPC: rclcpp handles convert_to_ros_message + convert_to_custom
// ---------------------------------------------------------------------------

static std::vector<double> bench_after(
  unsigned int size_x, unsigned int size_y, int n_iters, bool use_ipc)
{
  ...
  auto sub = node->create_subscription<Adapter>(
    ...
    [&recv_ns](std::shared_ptr<const nav2_costmap_2d::Costmap2DStamped>/*msg*/) {
      recv_ns.store(
...
// ---------------------------------------------------------------------------
// Before: Original code path (before this PR).
// CostmapSubscriber received nav2_msgs::msg::Costmap and called
// processCurrentCostmapMsg() on every message to produce a Costmap2D
// (malloc + full memcpy of the data buffer).
// convert_to_custom() is used here only as a stand-in for that work since
// it performs the identical malloc + memcpy — it is NOT from the original code.
// ---------------------------------------------------------------------------
...
static void run_suite(
  ...
  int n_iters, bool use_ipc)
{
  std::printf("  [%s] %s\n", suite_label, size_label);

  auto before = bench_before(size_x, size_y, n_iters, use_ipc);
  if (!before.empty()) {
    print_stats(before_label, before);
  } else {
    std::printf("    Before: NO SAMPLES\n");
  }

  auto after = bench_after(size_x, size_y, n_iters, use_ipc);
  if (!after.empty()) {
    print_stats(after_label, after);
  } else {
...
int main(int argc, char ** argv)
{
  ...
  std::printf("\n=== Costmap TypeAdapter Benchmark ===\n");
  std::printf("N=%d iterations per configuration (10 warm-up excluded)\n\n", N);
  std::printf(
    "Before: original code — nav2_msgs::msg::Costmap subscription + processCurrentCostmapMsg()\n"
    "        (simulated in bench_before() using convert_to_custom(), which does identical work)\n"
    "After:  this PR — TypeAdapter subscription; rclcpp manages conversion\n"
    "        With IPC: zero-copy (no malloc/memcpy); Without IPC: single convert_to_custom()\n\n");

  for (auto & s : sizes) {
    std::printf("[Map size: %s]\n", s.label);

    run_suite(
      "With IPC (intra-process, zero-copy)",
      "Before (standard msg + processCurrentCostmapMsg, IPC)",
      "After  (TypeAdapter, zero-copy IPC)",
      s.x, s.y, s.label, N, true);

    std::printf("\n");

    run_suite(
      "Without IPC (inter-process path)",
      "Before (standard msg + processCurrentCostmapMsg, no IPC)",
      "After  (TypeAdapter, no IPC)",
      s.x, s.y, s.label, N, false);
