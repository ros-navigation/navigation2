...
// Tests deterministic scenarios: no obstacles, vertex obstacle, edge obstacle, interior obstacle
TEST(collision_footprint, test_performance_benchmark) {
  ...
  // Create a rectangular footprint (2m x 1m)
  geometry_msgs::msg::Point p1;
  p1.x = -1.0;  // Left edge
  p1.y = -0.5;  // Bottom edge
  geometry_msgs::msg::Point p2;
  p2.x = 1.0;   // Right edge
  p2.y = -0.5;
  geometry_msgs::msg::Point p3;
  ...
  // Fixed test position (center of costmap)
  const double test_x = 12.5;  // Center of 25m x 25m map
  const double test_y = 12.5;
  const double test_theta = 0.0;
  ...
  // Scenario 0: Obstacle at a vertex of the polygon (fastest)
  // Place obstacle at the first vertex (p1: left edge, bottom edge)
  collision_checker.worldToMap(test_x - 1.0, test_y - 0.5, grid_x, grid_y);
  scenarios.push_back({"Obstacle at vertex", {{grid_x, grid_y}}});

  // Scenario 1: Obstacle on a line segment (edge)
  // Place obstacle on the last line segment (p4 to p1: top edge, from right to left)
  collision_checker.worldToMap(test_x - 0.5, test_y + 0.5, grid_x, grid_y);
  scenarios.push_back({"Obstacle on edge", {{grid_x, grid_y}}});

  // Scenario 2: Obstacle inside the polygon
  // Place obstacle at the center of the rectangle
  collision_checker.worldToMap(test_x, test_y, grid_x, grid_y);
  scenarios.push_back({"Obstacle inside", {{grid_x, grid_y}}});

  // Scenario 3: No obstacles (slowest)
  scenarios.push_back({"No obstacles", {}});
  ...
  for (const auto & scenario : scenarios) {
    ...
    // Test perimeter-only checking
    auto start_perimeter = std::chrono::high_resolution_clock::now();
    double perimeter_result = 0.0;
    for (int iter = 0; iter < num_iterations; ++iter) {
      perimeter_result = collision_checker.footprintCostAtPose(
        test_x, test_y, test_theta, footprint, false);
    }
    ...
    // Test full-area checking
    auto start_full_area = std::chrono::high_resolution_clock::now();
    double full_area_result = 0.0;
    for (int iter = 0; iter < num_iterations; ++iter) {
      full_area_result = collision_checker.footprintCostAtPose(
        test_x, test_y, test_theta, footprint, true);
    }
    ...
    if (scenario.name == "No obstacles") {
      EXPECT_NEAR(perimeter_result, 0.0, 0.001);
      EXPECT_NEAR(full_area_result, 0.0, 0.001);
    } else if (scenario.name == "Obstacle at vertex" || scenario.name == "Obstacle on edge") {
      // Both methods should detect obstacles on perimeter
      EXPECT_NEAR(perimeter_result, 254.0, 0.001);
      EXPECT_NEAR(full_area_result, 254.0, 0.001);
    } else if (scenario.name == "Obstacle inside") {
      // Only full-area should detect interior obstacles
      EXPECT_LT(perimeter_result, 254.0);  // Should not detect interior obstacle
      EXPECT_NEAR(full_area_result, 254.0, 0.001);  // Should detect interior obstacle
    }
