...
RotationShimController::transformPoseToBaseFrame(const geometry_msgs::msg::PoseStamped & pt)
{
  ...
  if (!nav2_util::transformPoseInTargetFrame(pt, pt_base, *tf_, costmap_ros_->getBaseFrameID())) {
    throw nav2_core::ControllerException("Failed to transform pose to base frame!");
  }
  return pt_base.pose;
}
  ...
  const geometry_msgs::msg::PoseStamped & pose)
{
  ...
  while (simulated_time < simulate_ahead_time_) {
    ...
    {
      throw nav2_core::ControllerException("RotationShimController detected a potential collision ahead!");
    }

    if (footprint_cost >= static_cast<double>(LETHAL_OBSTACLE)) {
      throw nav2_core::ControllerException("RotationShimController detected collision ahead!");
    }
  }
}
