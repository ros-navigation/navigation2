  ...
  const rclcpp::Time & t, const std::string & f)
{
  ...
  ident.header.frame_id = nav2_util::stripLeadingSlash(f);
...
{
  std::string laser_scan_frame_id = nav2_util::stripLeadingSlash(laser_scan->header.frame_id);
  ...
  if (lasers_update_[laser_index]) {
    ...
    min_q.header.frame_id = nav2_util::stripLeadingSlash(laser_scan->header.frame_id);
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::msg::PoseWithCovarianceStamped & msg)
{
  ...
  } else if (nav2_util::stripLeadingSlash(msg.header.frame_id) != global_frame_id_) {
    RCLCPP_WARN(
      get_logger(), "Ignoring initial pose in frame \"%s\"; initial poses must be in the global"
      " frame, \"%s\"",
      nav2_util::stripLeadingSlash(msg.header.frame_id).c_str(),
      global_frame_id_.c_str());
    return;
  }
...
AmclNode::initAmclParams()
{
  ...
  odom_frame_id_ = nav2_util::stripLeadingSlash(odom_frame_id_);
  base_frame_id_ = nav2_util::stripLeadingSlash(base_frame_id_);
  global_frame_id_ = nav2_util::stripLeadingSlash(global_frame_id_);
}
