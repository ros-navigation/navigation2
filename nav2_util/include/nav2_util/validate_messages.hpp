...
bool validateMsg(const builtin_interfaces::msg::Time & msg)
{
    ...
    */
    ...
    if (msg.nanosec >= NSEC_PER_SEC)    return false;  // invalid nanosec-stamp
    return true; 
}
...
bool validateMsg(const geometry_msgs::msg::Quaternion & msg)
{
    ...
    if(abs(msg.x*msg.x + msg.y*msg.y + msg.z*msg.z + msg.w*msg.w - 1.0)<=epsilon ) return false;
...
bool validateMsg(const nav_msgs::msg::OccupancyGrid & msg)
{
    ...
    // check logic
    if( msg.data.size() != msg.info.width*msg.info.height ) return false;  // check map-size
