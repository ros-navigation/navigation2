...
TEST(AnalyticExpansionTest, TestAnalyticExpansionNodesStructure)
{
  ...
  EXPECT_EQ(nodes.nodes[0].initial_coords.x, 1.0);
  EXPECT_EQ(nodes.nodes[0].initial_coords.y, 2.0);
  EXPECT_EQ(nodes.nodes[0].proposed_coords.x, 3.0);
  EXPECT_EQ(nodes.nodes[0].proposed_coords.y, 4.0);
}

// Test the countDirectionChanges method by directly calling it
TEST(AnalyticExpansionTest, TestCountDirectionChanges)
{
  {
