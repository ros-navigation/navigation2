...
bool AStarAlgorithm<NodeT>::areInputsValid()
{
  ...
  {
    throw nav2_core::GoalOccupied("Failed to compute path, goal is occupied with no tolerance.");
  }

  // Check if starting point is valid
  if (!_start->isNodeValid(_traverse_unknown, _collision_checker)) {
    throw nav2_core::StartOccupied("Starting point in lethal space! Cannot create feasible plan.");
  }
