...
planner_server:
  ros__parameters:
    ...
    GridBased:
      ...
      incremental_obstacle_heuristic: False  # For Hybrid/Lattice nodes: Maintain the goal-rooted obstacle heuristic field across replannings of the same goal and repair it incrementally (LPA*) for only the costmap cells that changed, instead of wiping and recomputing. Produces the same heuristic as a full recompute but is far cheaper when the costmap changes little between plans (dynamic replanning). Persists the field across cycles (subsumes cache_obstacle_heuristic) and always runs at full resolution (forces downsample_obstacle_heuristic off). Rebuilds fully on a goal change; the first plan to a new goal pays a full-field build.
