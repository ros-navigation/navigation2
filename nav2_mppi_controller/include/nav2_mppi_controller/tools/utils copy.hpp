  ...
  float rotation_threshold = 0.0f)
{
  if (path.poses.size() < 2) {
    RCLCPP_INFO(
      rclcpp::get_logger("mppi_utils"),
      "[MPPI PathUtils] Path too short for constraints (size=%zu)",
      path.poses.size());
    return path.poses.size();
  }

  unsigned int first_after_constraint = path.poses.size();

  // TYLKO DO LOGÓW: maksymalna rotacja poniżej progu (żeby wiedzieć, czy było \"prawie\")
  float max_rotation_below_threshold = 0.0f;
  ...
  if (rotation_threshold > 0.0f) {
    for (unsigned int idx = 0; idx < path.poses.size() - 1; ++idx) {
      ...
      if (translation < 1e-3) {
        ...
        // TYLKO DO LOGÓW: zapamiętaj największą rotację poniżej progu
        if (rotation < rotation_threshold &&
            rotation > max_rotation_below_threshold)
        {
          max_rotation_below_threshold = rotation;
        }
        ...
        if (rotation > rotation_threshold) {
          ...
          first_after_constraint = end_idx;

          // LOG: wykryto rotation constraint
          RCLCPP_INFO(
            rclcpp::get_logger("mppi_utils"),
            "[MPPI PathUtils] Rotation constraint detected: rotation=%.3f rad, "
            "threshold=%.3f rad, start_idx=%u, constraint_idx=%u",
            rotation, rotation_threshold, idx, end_idx);
  ...
  if (path.poses.size() >= 3) {
    for (unsigned int idx = 1; idx < path.poses.size() - 1; ++idx) {
      ...
      float norm_oa = hypotf(oa_x, oa_y);
      float norm_ab = hypotf(ab_x, ab_y);

      if (norm_oa < 1e-4f || norm_ab < 1e-4f) {
        continue;
      }

      float cos_ang = dot_product / (norm_oa * norm_ab);
      cos_ang = std::max(-1.0f, std::min(1.0f, cos_ang));
      float angle = acosf(cos_ang);  // [0, pi]

      if (rotation_threshold > 0.0f) {
        if (angle > rotation_threshold) {
          first_after_constraint = std::min(first_after_constraint, idx + 1);

          // LOG: constraint z powodu kąta (cusp/inversion po łuku)
          RCLCPP_INFO(
            rclcpp::get_logger("mppi_utils"),
            "[MPPI PathUtils] Inversion/cusp constraint detected by angle: "
            "angle=%.3f rad (threshold=%.3f rad), at idx=%u (constraint_idx=%u)",
            angle, rotation_threshold, idx, first_after_constraint);
        ...
        if (dot_product < 0.0f) {
          ...
          // LOG: klasyczny cusp przez dot_product<0
          RCLCPP_INFO(
            rclcpp::get_logger("mppi_utils"),
            "[MPPI PathUtils] Inversion cusp constraint detected by dot_product<0 "
            "at idx=%u (constraint_idx=%u, dot=%.6f)",
            idx, first_after_constraint, dot_product);
  ...
  // Podsumowanie: nic nie znalazł -> zobacz, czy rotacja była \"prawie\" wystarczająca
  if (first_after_constraint == path.poses.size()) {
    if (rotation_threshold > 0.0f && max_rotation_below_threshold > 0.0f) {
      float ratio = max_rotation_below_threshold / rotation_threshold;

      if (ratio >= 0.9f) {
        // LOG: rotacja była blisko progu (>= 90%)
        RCLCPP_INFO(
          rclcpp::get_logger("mppi_utils"),
          "[MPPI PathUtils] Rotation almost met threshold: rotation=%.3f rad, "
          "threshold=%.3f rad (%.1f%% of threshold) – no constraint applied",
          max_rotation_below_threshold, rotation_threshold, ratio * 100.0f);
      } else {
        RCLCPP_INFO(
          rclcpp::get_logger("mppi_utils"),
          "[MPPI PathUtils] No constraint found. Max rotation below threshold: "
          "rotation=%.3f rad, threshold=%.3f rad (%.1f%% of threshold)",
          max_rotation_below_threshold, rotation_threshold, ratio * 100.0f);
      }
    } else {
      RCLCPP_INFO(
        rclcpp::get_logger("mppi_utils"),
        "[MPPI PathUtils] No inversion / rotation constraint found "
        "(path_size=%zu, rotation_threshold=%.3f)",
        path.poses.size(), rotation_threshold);
    }
  }
  ...
  float rotation_threshold = 0.0f)
{
  ...
  const unsigned int first_after_constraint = findFirstPathInversion(cropped_path, rotation_threshold);
