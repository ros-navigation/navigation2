...
void CostCritic::score(CriticData & data)
{
  ...
  for (int i = 0; i < strided_traj_rows; ++i) {
    ...
    for (int j = 0; j < strided_traj_cols; j++) {
      ...
      int footprint_every_n_points_ = 2;
      ...
      bool use_footprint_cost = false;
      if (consider_footprint_) {
        const bool near_obstacle = (pose_cost >= possible_collision_cost_) || (possible_collision_cost_ < 1.0f);
        bool sample_this_point = false;
        if (footprint_every_n_points_ <= 0) {
          sample_this_point = (j == strided_traj_cols - 1);            // tylko końcowy punkt
        } else {
          sample_this_point = (j % footprint_every_n_points_ == 0) || (j == strided_traj_cols - 1);
        }
        use_footprint_cost = near_obstacle && sample_this_point;
      }
  ...
  //     RCLCPP_INFO(logger_, "[CostCritic] WLACZONO FOOTPRINT COST");
  //     RCLCPP_INFO(logger_,
  //   "[CostCritic] possible_collision_cost_ = %.1f",
  //   possible_collision_cost_); 
  //   } else { 
  //     RCLCPP_INFO(logger_, "[CostCritic] WYLACZONO FOOTPRINT COST"); 
  //     RCLCPP_INFO(logger_,
  //   "[CostCritic] possible_collision_cost_ = %.1f",
  //   possible_collision_cost_); 
  //   } 
