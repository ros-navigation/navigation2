...
void CostCritic::initialize()
{
  ...
  getParam(precollision_lookahead_time_, "precollision_lookahead_time", 1.0f); 
  getParam(precollision_penalty_, "precollision_penalty", 100.0f);
...
void CostCritic::score(CriticData & data)
{
  ...
  for (int i = 0; i < strided_traj_rows; ++i) {
    ...
    if (!trajectory_collide &&
        precollision_lookahead_time_ > 0.0f &&
        precollision_penalty_ > 0.0f)
    {
      // how many columns ahead correspond to precollision_lookahead_time_
      const unsigned int max_preview_cols = std::min(
        (unsigned int)std::ceil(
            precollision_lookahead_time_ /
            (data.model_dt * static_cast<float>(trajectory_point_step_))),
        (unsigned int)strided_traj_cols);

      bool collide_within_window = false;

      for (unsigned int pj = 0; pj < max_preview_cols; ++pj) {
        const float Px   = traj_x(i, pj);
        const float Py   = traj_y(i, pj);
        const float Pyaw = traj_yaw(i, pj);

        const double fp_cost = collision_checker_.footprintCostAtPose(
          (double)Px,
          (double)Py,
          (double)Pyaw,
          costmap_ros_->getRobotFootprint());

        // Jeśli footprint w tej próbce jest już w strefie naprawdę niebezpiecznej,
        // to ta trajektoria "uderzy" w przeszkodę w czasie krótszym niż
        // precollision_lookahead_time_.
        // Uznajemy ją za złą.
        if (fp_cost == (double)nav2_costmap_2d::LETHAL_OBSTACLE ||
            fp_cost == (double)nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
        {
          collide_within_window = true;
          break;
        }
      }

      if (collide_within_window) {
        // kara stała za to, że kolizja jest < precollision_lookahead_time_
        repulsive_cost(i) += precollision_penalty_;
      }
    }
