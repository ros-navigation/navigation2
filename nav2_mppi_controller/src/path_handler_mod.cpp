  ...
  const geometry_msgs::msg::PoseStamped & robot_pose)
{
    ...
    if (isWithinInversionTolerances(global_pose)) {
      ...
      global_plan_up_to_inversion_ = global_plan_;
        ...
        for (size_t i = 1; i < global_plan_.poses.size(); i++) {
          double yaw_prev = tf2::getYaw(global_plan_.poses[i-1].pose.orientation);
          double yaw_curr = tf2::getYaw(global_plan_.poses[i].pose.orientation);
          double diff = fabs(angles::shortest_angular_distance(yaw_prev, yaw_curr));
          if (diff > max_yaw_delta) {
            max_yaw_delta = diff;
          }
        }
      ...
      RCLCPP_INFO(
    logger_,
    "[MPPI PathHandler] setPath: rotation_threshold=%.3f, inversion_locale=%u, path_size=%zu",
    rotation_threshold,
    inversion_locale_,
  ...
  //   RCLCPP_INFO(
  // logger_,
  // "[MPPI PathHandler] step: enforce_path_inversion=%s, enforce_path_rotation=%s, inversion_locale=%u",
  // enforce_path_inversion_ ? "true" : "false",
  // enforce_path_rotation_ ? "true" : "false",
  // inversion_locale_);
