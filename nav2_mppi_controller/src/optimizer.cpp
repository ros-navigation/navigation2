  ...
  const xt::xtensor<float, 2> & sequence) const
{
  ...
  xt::noalias(traj_yaws) = cumsum_1d_op(wz * settings_.model_dt) + initial_yaw;
  ...
  xt::noalias(traj_x) = state_.pose.pose.position.x + cumsum_1d_op(dx * settings_.model_dt);
  xt::noalias(traj_y) = state_.pose.pose.position.y + cumsum_1d_op(dy * settings_.model_dt);
}
  ...
  const models::State & state) const
{
  ...
  xt::noalias(trajectories.yaws) =
    cumsum_2d_op(state.wz * settings_.model_dt, 1) + initial_yaw;
  ...
  xt::noalias(trajectories.x) = state.pose.pose.position.x +
    cumsum_2d_op(dx * settings_.model_dt, 1);
  xt::noalias(trajectories.y) = state.pose.pose.position.y +
    cumsum_2d_op(dy * settings_.model_dt, 1);
