...
class UpsamplerCostFunction : public ceres::FirstOrderFunction {
 public:
  ...
  struct CurvatureComputations 
  {
    ...
    Eigen::Vector2d d_k_d_xm1{0,0};
    Eigen::Vector2d d_k_d_xi{0,0};
    Eigen::Vector2d d_k_d_xp1{0,0};
  };
  ...
  virtual bool Evaluate(const double * parameters,
    ...
    for (uint i = 0; i != NumParameters() / 2; i++) {
      ...
      // compute cost
      addSmoothingResidual(3000, xi, xi_p1, xi_m1, cost_raw);  // TODO params
      addCurvatureResidual(60.0, xi, xi_p1, xi_m1, curvature_params, cost_raw);

    if (gradient != NULL) {

        // compute gradient
        addSmoothingJacobian(3000, xi, xi_p1, xi_m1, grad_x_raw, grad_y_raw);
        addCurvatureJacobian(60.0, xi, xi_p1, xi_m1, curvature_params, grad_x_raw, grad_y_raw);

        //TODO commenthing this out does help... but defeats somewhat the purpose 
