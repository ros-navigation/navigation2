...
SmacPlanner::~SmacPlanner()
{
  RCLCPP_INFO(_logger, "Destroying plugin %s of type SmacPlanner", _name.c_str());
}
...
{
  RCLCPP_INFO(_logger, "Activating plugin %s of type SmacPlanner", _name.c_str());
...
{
  RCLCPP_INFO(_logger, "Deactivating plugin %s of type SmacPlanner", _name.c_str());
...
{
  RCLCPP_INFO(_logger, "Cleaning up plugin %s of type SmacPlanner", _name.c_str());
  _a_star.reset();
  _smoother.reset();
  ...
  const geometry_msgs::msg::PoseStamped & start, const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  if (!_smoother->smooth(path_world, costmap, _smoother_params)) {
    RCLCPP_WARN(
      _logger, "%s: failed to smooth plan, Ceres could not find a usable solution to optimize.",
      _name.c_str());
    return plan;
  }
