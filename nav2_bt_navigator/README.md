...
#### Control Nodes
* FirstCompletion: This is a variant of a Parallel Node. When this node is ticked,
it will tick all of its sub-chilren. If any child returns SUCCESS or FAILURE, it
will halt the others and return that status. Otherwise, it returns RUNNING. In other words, this node allows
multiple substrees to be running in parallel. The first tree to finish gets it's
status returned.
