...
TEST(SimpleNonChargingDockTests, DockedTransformFreshness)
{
...
TEST(SimpleNonChargingDockTests, DetectionUsesMeasurementTransform)
{
  auto node = std::make_shared<nav2::LifecycleNode>("test");
  ...
  SimpleNonChargingDockTestable dock;
  dock.configure(node, "my_dock", tf_buffer);
  dock.activate();
