...
TEST(SimpleChargingDockTests, SubscriptionCallback)
{
  ...
  int max_tries = 10;
  while (publisher->get_subscription_count() == 0 && max_tries-- > 0) {
    rclcpp::spin_some(node->get_node_base_interface());
    std::this_thread::sleep_for(100ms);
  }
  ASSERT_GT(publisher->get_subscription_count(), 0);

  // Publish and spin to process the message
...
TEST(SimpleChargingDockTests, DetectorServiceTimeout)
{
  ...
  // The call to invoke() should timeout and be caught, not crash the test.
